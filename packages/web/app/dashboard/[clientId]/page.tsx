...
const MACRO_LABELS: Record<string, { label: string; unit: string; color: string }> = {
  calories:  { label: "Calories",  unit: "kcal", color: "text-sage-600" },
  protein_g: { label: "Protein",   unit: "g",    color: "text-sky-600" },
  carbs_g:   { label: "Carbs",     unit: "g",    color: "text-amber-600" },
  fat_g:     { label: "Fat",       unit: "g",    color: "text-rose-600" },
};
  ...
  return (
    <div className="px-8 py-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              {connected && <span className="dot-live" />}
            </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {Object.entries(MACRO_LABELS).map(([key, meta]) => (
            <div key={key} className="card px-4 py-4">
              <p className={`text-2xl font-semibold tracking-tight ${meta.color}`}>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <StatCard label="Workouts" value={String(activityLogs.length)} />
        <StatCard label="Weigh-ins" value={String(biometricLogs.length)} />
      </div>
    </div>
  );
}
